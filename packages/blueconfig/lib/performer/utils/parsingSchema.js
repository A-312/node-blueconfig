...
const SchemaNode = require('./../../model/schemanode.js')
...
function parsingSchema(name, rawSchema, schemaObjectModel, fullpath) {
  if (name === '_cvtProperties') {
    throw new SCHEMA_INVALID(unroot(fullpath), "'_cvtProperties' is reserved word of blueconfig, it can be used like property name.")
  }
  ...
  if (isObjNotNull(rawSchema) && !isArray && countChildren > 0 &&
    ...
    Object.keys(rawSchema).forEach((key) => {
      ...
      parsingSchema.call(this, key, rawSchema[key], schemaObjectModel[name]._cvtProperties, path)
    })
    return
  ...
  const newFormat = (() => {
    if (BUILT_INS.indexOf(format) >= 0 || BUILT_IN_NAMES.indexOf(format) >= 0) {
      ...
      if (!this.Ruler.types.has(format)) {
        throw new SCHEMA_INVALID(unroot(fullpath), `uses an unknown format type (current: ${JSON.stringify(format)})`)
      }
      ...
      throw new SCHEMA_INVALID(unroot(fullpath), `${errorMessage} (current: ${JSON.stringify(value)})`)
